    ...
    },
    {
        ...
        handler: (request, response) => __awaiter(void 0, void 0, void 0, function* () {
            try {
                ...
                const downloadStream = bucket.openDownloadStream(request.params.attached_file_name);
                const downloadfile = downloadStream.pipe(fs_1.default.createWriteStream(request.params.attached_file_name));
                return response.response({ status: "ok", data: downloadfile });
            }
            catch (err) {
                return response
                    .response({ status: "ok", err: "Download failed" })
                    .code(501);
            }
        }),
    },
